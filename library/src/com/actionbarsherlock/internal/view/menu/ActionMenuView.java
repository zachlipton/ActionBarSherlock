...
public class ActionMenuView extends LinearLayout implements MenuView, MenuBuilder.ItemInvoker {
	...
	public ActionMenuView(Context context, AttributeSet attrs) {
		...
		//mReserveOverflow = (getResources().getConfiguration().screenLayout & Configuration.SCREENLAYOUT_SIZE_MASK) >= Configuration.SCREENLAYOUT_SIZE_LARGE;
	...
	*/
	...
	private int getMaxActionButtons() {
		return getResources().getInteger(R.integer.abs__max_action_buttons);
	}
	
	//private static boolean isDivider(View view) {
	//	return (view != null) && (view.getId() == R.id.abs__action_menu_divider);
	//}
	...
	private ImageView makeDividerView() {
		...
		view.setId(R.id.abs__action_menu_divider);
		return view;
	}
	
	//private boolean removeChildrenUntil(int startAt, View targetChild, boolean needsPreDivider) {
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		//mReserveOverflow = (newConfig.screenLayout & Configuration.SCREENLAYOUT_SIZE_MASK) >= Configuration.SCREENLAYOUT_SIZE_LARGE;
		mMaxItems = getMaxActionButtons();
		mWidthLimit = getResources().getDisplayMetrics().widthPixels / 2;
		
		if (mMenu != null) {
			mMenu.setActionWidthLimit(mWidthLimit);
	...
	public void updateChildren(boolean cleared) {
		List<MenuItemImpl> menuItems = mMenu.getActionItems(false/*mReserveOverflow*/);
